...
I particularly don't care about the usual metrics, such as [Frechet Inception Distance (FID)](https://nealjean.com/ml/frechet-inception-distance/), so we will turn this off, especially since they usually take a long time to compute.
...
```
python run_training.py --num-gpus=1 --data-dir=/path/to/datasets/root/ --dataset=nameofyourdataset \
	 --config=config-f --mirror-augment=true --metrics=none
```
